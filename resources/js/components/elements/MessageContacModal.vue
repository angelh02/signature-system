<template>
    <div>
        <div class="modal fade" :id="modalId != null && modalId != ''? modalId : 'contact-modal'" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="staticBackdropLabel">Mensaje de contacto</h5>
                    </div> <!-- end modal header -->
                    <div class="modal-body">
                        <!-- Email del mensaje -->
                        <div class="mb-2" v-if="user?.email != null">
...
import useDocumentRequest from "@/api/document/index.js";
...
function storeRequest(){
    if(!(v$._value.$errors.length > 0 || v$._value.$silentErrors.length > 0)){
        ...
        useDocumentRequest.contactOwner(data)
        .then((res) => {
            toast.success("Se ha mandado el mensaje correctamente", {
            timeout: 2000,
            });
            emit('stored');
        })
        .catch(error => 
            toast.warning("No se ha mandar el mensaje", {
            timeout: 2000,
            })
        );
    }
